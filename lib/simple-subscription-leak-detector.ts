...
class SimpleSubscriptionLeakDetector {
  private recorder = new Recorder();
  private simpleSubscriptionLeakReporter = new SimpleSubscriptionLeakReporter();
  ...
  handlePageChange() {
    if (this.recorder.recording) {
      ...
      if (oldSnapshot) {
        console.log(this.simpleSubscriptionLeakReporter.report(snapshot, oldSnapshot));
      }
